...
# Fedora uses systemd unconditionally — there is no init-system prompt.
...
if [[ "$DEVICE_CHOICE" == "1" ]]; then
  ...
  # Standard Fedora packages (no init-system suffixes needed under systemd)
  LAPTOP_PKGS=(brightnessctl acpi powertop bluez tlp acpid kernel-tools)

  sudo dnf install -y "${LAPTOP_PKGS[@]}" ||
    die "Failed to install laptop packages."

  # Enable & start systemd services
  for svc in tlp acpid bluetooth; do

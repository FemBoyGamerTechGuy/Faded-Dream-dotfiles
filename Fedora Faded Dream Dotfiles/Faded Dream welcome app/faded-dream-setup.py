...
# Lives in ~/Faded-Dream-dotfiles/ArchLinux Faded Dream Dotfiles/Faded Dream welcome app/faded-dream-setup.py
...
from packages import (BROWSERS, GAMING, FILE_MANAGERS, GITHUB_MAP, FLATPAK_MAP, OTHER, LANG_LIST, LO_CODES, TB_CODES,
                      ...
                      REPO_STYLE, COPR_MAP, HYPRLAND_CONF, SENTINEL_FILE,
...
class SetupWindow(Adw.ApplicationWindow):
    ...
    def _on_install(self, _btn):
        ...
        copr_pkgs    = [p for p in all_pkgs if     COPR_MAP.get(p, False)]
        repo_pkgs    = [p for p in all_pkgs if not COPR_MAP.get(p, False) and p not in GITHUB_MAP and p not in FLATPAK_MAP]
        ...
        if copr_pkgs:
            self._log_append(T('log_aur_line', n=len(copr_pkgs), pkgs=', '.join(copr_pkgs)), 'aur')
        if self.browser:
            self._log_append(T('log_browser_patch', exec=self.browser['exec']), 'patch')
        self._log_append("", "raw")

        threading.Thread(target=self._install_thread, args=(repo_pkgs, copr_pkgs, github_pkgs, flatpak_pkgs), daemon=True).start()

    def _install_thread(self, repo_pkgs, copr_pkgs, github_pkgs=None, flatpak_pkgs=None):
        github_pkgs  = github_pkgs  or []
        flatpak_pkgs = flatpak_pkgs or []
        all_pkgs     = repo_pkgs + copr_pkgs + github_pkgs
        total = max(len(repo_pkgs) + len(copr_pkgs) + len(github_pkgs) + len(flatpak_pkgs) + (1 if self.browser else 0), 1)
        done  = [0]
        ...
        if repo_pkgs:
            ...
            stream(["sudo","dnf","install","-y","--color=never"] + repo_pkgs, "repo")
            done[0] += len(repo_pkgs)

        for pkg in copr_pkgs:
            self._log_append(T('log_banner_paru', pkg=pkg), 'header')
            ui(T('prog_pkg', pkg=pkg))
            stream(["sudo","dnf","install","-y","--color=never", pkg], "aur")
            done[0] += 1
        ...
        if flatpak_pkgs:
            if not shutil.which("flatpak"):
                ...
                stream(["sudo","dnf","install","-y","flatpak"], "repo")
            else:
        ...
        cups_installed = any(p.startswith("cups") for p in (repo_pkgs + copr_pkgs))
        if cups_installed:
            init = detect_init()
            self._log_append(T("log_cups_start"), "header")
            if init == "systemd":
                subprocess.run(["sudo", "systemctl", "enable", "--now", "cups.service"], check=False)
                self._log_append(T("log_cups_systemd"), "patch")
            elif init == "runit":
